 ...
 *   - When interating over laps in the render method, we need a key prop. Currently there is nothing to make it unique. One solution is to turn state.laps into an array of objects with a key attribute, and a value. When we create laps, we could use an internal counter.
 *
 *   Formatting:
 *   - Returning a function in handleDeleteClick is unnecessary since there is no need for a closure. I wouldn't mix functional with non-functional styles when there is no good reason for it. (Be sure to pass it as () => this.handleDeleteClick(i) in Laps.onDelete)
 *   - No need to pass an array to laps.concat when appending a single value; just pass the value itself.
 *   - Set incrementer back to null after clearInterval for consistency, as it represents the current interval. This negates the need for lastClearedIncrementer, as it's only being used for comparing it to incrementer to check if the timer is running. Now we can simply check if 'incrementer' is null or not, instead.
